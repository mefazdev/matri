...
export default function LocEdit({id}) {
          ...
          <div className="basic__edit__row grid md:grid-cols-3 lg:grid-cols-4">
            ...
            <select onChange={(e)=>setdistrict(e.target.value)}>
               ...
               {districts?.map((dist,index)=>{
                return(
                  <option selected={district == dist  ? true :false } key={index}>{dist}</option>
                )
               })}
            </select>
          </div>
          ...
          <div className="basic__edit__row grid md:grid-cols-3 lg:grid-cols-4">
            ...
            <select onChange={(e)=>setcity(e.target.value)}>
            {
              cities?.map((cty,index)=>{
                return(
                  <option selected={city == cty ? true :false} key={index}>{cty}</option>
                )
              })
  ...
  />
         ...
         <div className="basic__edit__row grid md:grid-cols-3 lg:grid-cols-4">
            ...
            <input value={callTime} onChange={(e)=>setcallTime(e.target.value)} />
          </div>
          <div className="basic__edit__row grid md:grid-cols-3 lg:grid-cols-4">
